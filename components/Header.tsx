...
import { Search, ShoppingCart, Truck, Phone, User, Menu, ChevronDown, Package, Wand2 } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onOpenGenerator }) => {
  const [isMegaMenuOpen, setIsMegaMenuOpen] = useState(false);

  return (
    <header className="flex flex-col w-full relative z-50 font-sans">
      {/* Top Utility Bar - Professional Dark Blue */}
      <div className="bg-[#0f172a] text-xs py-2 px-4 text-white font-medium flex justify-between items-center border-b border-gray-800 hidden md:flex">
        ...
        <div className="flex items-center gap-6">
             <button onClick={onOpenGenerator} className="flex items-center gap-1 text-yellow-400 hover:text-yellow-300 font-bold">
                <Wand2 size={12}/> Marketing Tools
            </button>
            <span className="text-gray-400">|</span>
            <a href="#business" className="hover:text-gray-300 transition-colors">Business Accounts</a>
            <span className="text-gray-400">|</span>
      ...
      <div className="bg-white text-gray-900 py-6 px-4 md:px-8 border-b border-gray-200 shadow-sm relative z-20">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-8">
          ...
          {/* Search Bar - Professional/Standard */}
          ...
          <div className="flex items-center gap-6 shrink-0">
            ...
            <a href="#orders" className="hidden md:flex flex-col items-center group text-gray-600 hover:text-[#0f172a]">
            ...
            <a href="#account" className="flex flex-col items-center group text-gray-600 hover:text-[#0f172a]">
            ...
            <a href="#cart" className="flex flex-col items-center group text-gray-600 hover:text-[#0f172a] relative">
              ...
              <div className={`absolute top-full left-0 pt-0 w-[1000px] transition-all duration-200 ${isMegaMenuOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
                  <div className="bg-white text-gray-800 shadow-2xl grid grid-cols-4 border-t-4 border-[#dc2626]">
                        ...
                        <div key={idx} className="p-6 border-r border-gray-100 last:border-0 bg-white flex flex-col h-full">
                            ...
                            <ul className="space-y-3 mb-6 flex-1">
                                {section.items.map((item, i) => (
                                    <li key={i}>
                                        <a href="#" className="text-sm text-gray-600 hover:text-[#dc2626] hover:underline block transition-colors">{item}</a>
                                    </li>
                                ))}
                            </ul>
                            {/* Featured Image in Menu */}
                            <div className="mt-auto group/img cursor-pointer relative overflow-hidden rounded-md h-32">
                    ...
                    <div className="col-span-4 bg-gray-50 p-4 border-t border-gray-200 flex justify-between items-center">
                        ...
                        <a href="#" className="text-sm font-bold text-[#dc2626] hover:underline">Request Quote &rarr;</a>
                    </div>
                  </div>
              </div>
            </div>
